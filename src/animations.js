export const headerAnimation = {
    hidden: { x: -400, opacity: 0 },
    show: { x:0, opacity: 1 }, /* fade from left to right */
};

export const headerImageAnimation = {
    hidden: { x: 200, opacity: 0 },
    show: { x:0, opacity: 1 }, /* fade from right to left */
};

export const rightHeaderAnimation = {
    hidden: { x: 200, opacity: 0 },
    show: { x:0, opacity: 1 }, /* fade from right to left */
};

export const centerHeaderAnimation = {
    hidden: { y: 100, opacity: 0 },
    show: { y:0, opacity: 1 }, /* fade from bottom to top */
}

export const leftHeaderAnimation = {
    hidden: { x: -400, opacity: 0 }, /* fade from right to left */
    show: { x:0, opacity: 1 },
};

export const ctaAnimation = {
    hidden: { y: 50, opacity: 0 },
    show: { y:0, opacity: 1 }, /* fade from bottom to top */
};

export const aboutParagraphAnimation = {
    hidden: { y: 50, opacity: 0 },
    show: { y:0, opacity: 1 }, /* fade from bottom to top */
};

export const aboutCardsAnimation = {
    hidden: { y: 200, opacity: 0 },
    show: { y:0, opacity: 1 }, /* fade from bottom to top */
};

export const aboutImageAnimation = {
    hidden: { x: -400, opacity: 0 },
    show: { x:0, opacity: 1 }, /* fade from left to right */
};

export const experienceCardsAnimation = {
    hidden: { scale: 0, opacity: 0 }, /* scale from 0 to 1 */
    show: { scale:1, opacity: 1 },
};

export const servicesAnimation = {
    hidden: { y: 100, opacity: 0 },
    show: { y:0, opacity: 1 }, /* fade from bottom to top */
};

export const portfolioCardsAnimation = {
    hidden: { scale: 0, opacity: 0 }, /* scale from 0 to 1 */
    show: { scale:1, opacity: 1 },
};
